...
public class WinningShotAI extends AIPlayer {
	...
	public Grid getCoupPerdant(Grid C) {
		...
		coupPerdant.set(coupPerdant.width() - 1, coupPerdant.height() - 1, true); // seule la dernière case n'est pas
																					// mangée
		return coupPerdant;
	}
	...
	public Grid findWinningShot(Grid C) { /*
											 * renvoie la configuration la meilleure selon le principe du coup gagnant
											 * (ou une tirée au hasard parmi les meilleures si il y en a plusieurs) en
											 * prenant la grille courante en argument
											 */
	...
	public boolean tempsEcoule() {
		int coordinates[] = adaptateurGridIndice(this.g, findWinningShot(this.g));
		System.out.println("L'IA a jou� : x = " + coordinates[0] + " y = " + coordinates[1]);
		this.g.play(coordinates[0], coordinates[1]);
		return true;
	}
